...
#define NETWORK_STRUCT_INIT(network) {.num_current_readers = 0, .num_current_writers = 0,.lock = __SPIN_LOCK_UNLOCKED(lock),.readers_queue = DECLARE_WAIT_QUEUE_HEAD(readers_queue),.writers_queue = __WAIT_QUEUE_HEAD_INITIALIZER(writers_queue), .num_waiting_readers = 0, .num_waiting_writers = 0}
